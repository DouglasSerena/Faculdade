...
db.metros.insertOne({ _id: metro_ref, nome: "Super car", quantidada_maxima_pessoa: 20 });
db.passageiros.insertOne({ _id: epassageiro_ref, nome: "Douglas", cpf: "00099900099" });
db.enderecos.insertOne({ _id: endereco_ref, rua: "Tres patos", numero: "1234", bairro: "Zona nova", cep: "95555000" });
db.estacoes.insertOne({ _id: estacao_ref, nome: "viente", endereco_ref: endereco_ref });
db.viagens.insertOne({ _id: viagem_ref, data_inicio: new ISODate(), data_fim: new ISODate(), status: "AGUARDANDO", metro_ref: metro_ref, estacao_ref: estacao_ref });
db.tickets.insertOne({ valor: 23, data_compra: new ISODate(), utilizado: false, epassageiro_ref: epassageiro_ref, viagem_ref: viagem_ref });
