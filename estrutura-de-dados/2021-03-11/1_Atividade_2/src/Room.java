  ...
  public void showNoteMoreMedia() {
    ...
    for (int index = 0; index < student; index++) {
      if (this._note[index] >= media) {
        System.out.println("O aluno " + this._student[index] + " teve a nota " + this._note[index]
            + " sendo maior que a media de " + media);
      }
    }
  }

  public void showNoteLessMedia() {
    double media = this.calcMedia();
    ...
    for (int index = 0; index < student; index++) {
      if (this._note[index] <= media) {
        System.out.println("O aluno " + this._student[index] + " teve a nota " + this._note[index]
            + " sendo menor que a media de " + media);
      }
    }
  }
